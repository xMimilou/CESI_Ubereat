    ...
    methods: {
        ...
        axios.post(request, {}, { headers: { 'auth-token': token } }).then((response) => {
            ...
            for(var i = 0; i < this.lists.length; i++){
                // add new element as json format without push
                this.lists[i][this.headinglength] = `<a href="/edit/${this.lists[i][0]}">Edit</a>`
                this.lists[i][this.headinglength + 1] = `<a href="/delete/${this.lists[i][0]}">Delete</a>`
